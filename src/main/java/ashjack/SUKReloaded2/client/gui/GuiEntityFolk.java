...
import ashjack.SUKReloaded2.folk.FolkRelationships;

public class GuiEntityFolk extends GuiScreen {
	private static final ResourceLocation myBackgroundTexture = new ResourceLocation(
			"suk2", "textures/gui/guiFolk.png");

	private int currentPage = 0;
	private int mouseCount = 0;
	private FolkData theFolk;
	private EntityPlayer entityplayer;
	private ArrayList<FolkRelationships> folksRelationships;
	...
	public void drawScreen(int i, int j, float f) {
		...
		if (currentPage == 0) {
			...
			String[] name = Minecraft.getMinecraft().thePlayer.toString()
					.split("'");
			...
			fontRendererObj.drawString(theFolk.identity.folkName, sec, 27, 0x000080);
			fontRendererObj.drawString("Age", left, 37, 0x000000);

			if (theFolk.identity.folkAge > 1) {
				fontRendererObj.drawString(theFolk.identity.folkAge + " years old", sec, 37,
						0x000080);
			} else if (theFolk.identity.folkAge == 1) {
				fontRendererObj.drawString("1 year old", sec, 37, 0x000080);
			} else {
			...
			if (theFolk.identity.folkGender == 0) {
				words = "Male ";
			} else {
				words = "Female ";
			}

			if (theFolk.identity.folkAge >= 18) {
				words += "adult";
			} else {
				words += "child";
			}
			...
			fontRendererObj.drawString("Race", left, 57, 0x000000);

			if (theFolk.identity.folkAge >= 18) {
				fontRendererObj.drawString(theFolk.race.raceName, sec, 57, 0x000080);
			} else {
			...
			fontRendererObj.drawString("Homeless", sec, 67, 0x000080);
			...
			fontRendererObj.drawString("Building skill level", left, 97,
					0x000000);
			fontRendererObj.drawString((int) theFolk.levelBuilder + " of 10",
					sec, 97, 0x000080);
			...
			fontRendererObj.drawString("Mining skill level", left, 107,
					0x000000);
			fontRendererObj.drawString((int) theFolk.levelMiner + " of 10",
					sec, 107, 0x000080);
			...
			fontRendererObj.drawString("Soldier skill level", left, 117,
					0x000000);
			fontRendererObj.drawString((int) theFolk.levelSoldier + " of 10",
					sec, 117, 0x000080);
			...
			if (theFolk.pregnancyStage > 0.0f) {
				fontRendererObj.drawString("Medical status", left, 127,
						0x000000);
				...
				fontRendererObj.drawString(days, sec, 127, 0x000080);
			}*/
			...
			int disoffset = 30;
			drawCenteredString(fontRendererObj, theFolk.identity.folkName
					+ "'s Relationships", width / 2, 10, 0xFFFFFF);

			/*for (int r = relOffset; r < folksRelationships.size(); r++) {
				try {
					FolkRelationships rel = folksRelationships.get(r);
					String[] sp = rel.toStringPersepctive(theFolk).split(": ");
					fontRendererObj
							.drawString(sp[0], left, disoffset, 0x000000);
					fontRendererObj.drawString(sp[1], this.width / 2,
							disoffset, 0x000080);
					disoffset += 10;
			...
			int disoffset = 30;
			drawCenteredString(fontRendererObj, theFolk.identity.folkName + "'s Needs",
					width / 2, 10, 0xFFFFFF);

			fontRendererObj.drawString("Hunger:", left, 30, 0x000000);
			fontRendererObj.drawString(String.valueOf(theFolk.needs.hungerLevel), this.width / 2, 30,
					0x000080);

			/*
			 * fontRendererObj.drawString("Social:", left, 50, 0x000000);
			 * fontRendererObj.drawString(theFolk.socialStatus, this.width / 2,
			 * 50, 0x000080);
			 * 
			 * fontRendererObj.drawString("Fun:", left, 70, 0x000000);
			 * fontRendererObj.drawString(theFolk.funStatus, this.width / 2, 70,
			 * 0x000080);
			 * 
			 * fontRendererObj.drawString("Environment:", left, 90, 0x000000);
			 * fontRendererObj.drawString(theFolk.environmentStatus, this.width
			 * / 2, 90, 0x000080);
			 */
	...
	private void showPage() {
		...
		if (currentPage == 0) {
			...
			buttonList.add(new GuiButton(1, 2, height - 42, 50, 20, "Back"));

			if (relOffset > 0) {
				buttonList
						.add(new GuiButton(2, width / 2 - 125, 8, 20, 20, "<"));
			}
			...
			if (rels - relOffset > 18) {
				buttonList
						.add(new GuiButton(3, width / 2 + 105, 8, 20, 20, ">"));
			}
		} else if (currentPage == 2) {
			buttonList.add(new GuiButton(1, 2, height - 42, 50, 20, "Back"));
		}
	...
	protected void actionPerformed(GuiButton guibutton) {
		...
		if (currentPage == 0) {
			...
			if (guibutton.displayString.contentEquals("Inventory")) {
				...
				// player.openGui(ModSimukraft.instance,
				// ModSimukraft.dynamicGuiID, player.worldObj, (int)
				// player.posX, (int) player.posY, (int) player.posZ);
				// /ModSimukraft.packetPipeline.sendToServer(new
				// OpenFolkInventoryPacket(References.GUI_FOLKINVENTORY));
			}
